...
installbbr(){
	...
	detele_kernel
	BBR_grub
	echo -e "${Tip} 重新啟動VPS後，請重新執行腳本開啟${Red_font_prefix}BBR/BBR魔改版${Font_color_suffix}"
	stty erase '^H' && read -p "需要重啟VPS後，才能開啟BBR/BBR魔改版，現在是否重新啟動 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重啟中..."
		reboot
...
installbbrplus(){
	...
	detele_kernel
	BBR_grub
	echo -e "${Tip} 重新啟動VPS後，請重新執行腳本開啟${Red_font_prefix}BBRplus${Font_color_suffix}"
	stty erase '^H' && read -p "需要重新啟動VPS後，才能開啟BBRplus，是否現在重新啟動 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重啟中..."
		reboot
...
installlot(){
	...
	detele_kernel
	BBR_grub
	echo -e "${Tip} 重新啟動VPS後，請重新執行腳本開啟${Red_font_prefix}Lotserver${Font_color_suffix}"
	stty erase '^H' && read -p "需要重新啟動VPS後，才能開啟Lotserver，是否現在重新啟動 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重啟中..."
		reboot
...
*               hard    nofile          1000000">/etc/security/limits.conf
	...
	read -p "需要重新啟動VPS後，才能生效系統最佳化配置，是否現在重新啟動 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重啟中..."
		reboot
...
start_menu(){
clear
echo && echo -e " TCP加速 一键安裝管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  
 ${Green_font_prefix}0.${Font_color_suffix} 升级脚本
————————————内核管理————————————
 ${Green_font_prefix}1.${Font_color_suffix} 安裝 BBR/BBR魔改版内核
 ${Green_font_prefix}2.${Font_color_suffix} 安裝 BBRplus版内核 
 ${Green_font_prefix}3.${Font_color_suffix} 安裝 Lotserver(锐速)内核
————————————加速管理————————————
...
————————————————————————————————" && echo
	...
	if [[ ${kernel_status} == "noinstall" ]]; then
		echo -e " 當前狀態: ${Green_font_prefix}未安裝${Font_color_suffix} 加速内核 ${Red_font_prefix}请先安裝内核${Font_color_suffix}"
	else
		echo -e " 當前狀態: ${Green_font_prefix}已安裝${Font_color_suffix} ${_font_prefix}${kernel_status}${Font_color_suffix} 加速内核 , ${Green_font_prefix}${run_status}${Font_color_suffix}"
		
	fi
echo
read -p " 請輸入數字 [0-11]:" num
case "$num" in
	0)
	Update_Shell
	;;
...
detele_kernel(){
	if [[ "${release}" == "centos" ]]; then
		...
		if [ "${rpm_total}" > "1" ]; then
			...
			echo -e " 检测到 内核 数量不正确，请檢查 !" && exit 1
		fi
	elif [[ "${release}" == "debian" || "${release}" == "ubuntu" ]]; then
	...
	if [[ "${release}" == "centos" ]]; then
        if [[ ${version} = "6" ]]; then
            if [ ! -f "/boot/grub/grub.conf" ]; then
                echo -e "${Error} /boot/grub/grub.conf 找不到，请檢查."
                exit 1
            fi
            sed -i 's/^default=.*/default=0/g' /boot/grub/grub.conf
        elif [[ ${version} = "7" ]]; then
            if [ ! -f "/boot/grub2/grub.cfg" ]; then
                echo -e "${Error} /boot/grub2/grub.cfg 找不到，请檢查."
                exit 1
            fi
            grub2-set-default 0
        fi
